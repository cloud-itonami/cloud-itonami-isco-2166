(ns design-studio.actor
  "DesignOpsActor — the ISCO-08 2166 independent graphic/multimedia designer
  ...
  The unconditional invariant: the DesignOpsAdvisor can never directly
  commit a record or use an asset the GraphicDesignGovernor refuses — every
  commit-record! call is gated behind `:decide`."
  (:require [design-studio.advisor :as advisor]
...
(defn build-graph
  "Build a compiled DesignOpsActor graph. `store` implements
  `design-studio.store/Store`. `advisor` implements
  `design-studio.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn approve!
  ...
  license verification / brand decision)."
  [graph thread-id]
