(ns design-studio.store
  "SSoT for the ISCO-08 2166 independent graphic/multimedia designer
  sole-proprietor actor. Store is a protocol injected into the
  `design-studio.actor` StateGraph — `MemStore` is the default,
  ...
  Domain:
    ...
    project  — a registered client project (:project/id, the client's
               :licensed-assets set, :brand-guidelines)
    record   — a committed operating record under a project (proof
               production, design delivery) — written ONLY via
...
(defrecord MemStore [a]
  ...
  (records-of [_ project-id]
    (filter #(= project-id (:project-id %)) (:records @a)))
  (ledger [_] (:ledger @a))
  (register-project! [s project]
